...
% patchSize    : 3-D crop fed to the network (avoids loading full volumes into GPU)
% patchOverlap : overlap on each side at inference (stride = patchSize - 2*overlap)
% patchesPerVolume: independent random patches drawn from each volume per epoch
TRAIN_OPTS = struct( ...
    'patchSize',          [64 64 64], ...
    'patchOverlap',       [8  8  8 ], ...
    'patchesPerVolume',   8,          ...
...
if ~isempty(poolIdx)
    ...
    w  = double(extractdata(pl.logWeights));
...
evalOpts.patchSize    = TRAIN_OPTS.patchSize;
