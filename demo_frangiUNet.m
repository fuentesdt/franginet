...
base_opts = struct( ...
    ...
    'plots',              'none', ...
    'numFrangiChannels',  2, ...
    'lambdaConsistency',  0.1, ...   % MSE(Y_pred, V) weight  (frangi_unet_consistency)
    'lambdaBreak',        0.1 ...    % |∇V|*(1-Y) break-penalty weight (frangi_unet_consistency)
);
...
RUN_MODELS = {'unet', 'frangi_unet_consistency'};   % <── edit here to select a subset

ALL_MODELS = {
    'Plain U-Net',              struct('archMode','unet'); ...
    'Hybrid Frangi-UNet',       struct('archMode','frangi_unet'); ...
    'FUNet cons+break',         struct('archMode','frangi_unet_consistency', ...
                                       'lambdaConsistency',0.1,'lambdaBreak',0.1); ...
    'FUNet cons only',          struct('archMode','frangi_unet_consistency', ...
                                       'lambdaConsistency',0.1,'lambdaBreak',0.0); ...
    'FUNet break only',         struct('archMode','frangi_unet_consistency', ...
                                       'lambdaConsistency',0.0,'lambdaBreak',0.1); ...
    'FUNet no penalty',         struct('archMode','frangi_unet_consistency', ...
                                       'lambdaConsistency',0.0,'lambdaBreak',0.0); ...
    'Frangi threshold',         struct('archMode','frangi_threshold'); ...
    'Frangi learnable thr',     struct('archMode','frangi_learnable_threshold'); ...
...
for m = 1:nModels
    ...
    if ismember(mopts.archMode, {'frangi_threshold','frangi_learnable_threshold'})
        initOpts          = mopts;
...
frangiArchs = {'frangi_unet','frangi_unet_consistency','frangi_threshold','frangi_learnable_threshold','frangi_linear','frangi_multichannel'};
